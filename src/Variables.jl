...
function read_number_of_nodal_variables(exo_id::int)
    num_vars = Ref{IntKind}(0)
    ex_get_variable_param!(exo_id, EX_NODAL, num_vars)
    return num_vars[]
end

function read_nodal_variable_names!(exo_id::int, num_vars::IntKind, var_name::Vector{UInt8}, var_names::Vector{String})
    for n = 1:num_vars
...
function read_nodal_variable_names(exo_id::int)
...
function read_nodal_variable_values(exo_id::int, time_step::IntKind, variable_index::IntKind, num_nodes::IntKind)
...
function write_number_of_nodal_variables(exo_id::int, num_vars::IntKind)
    ex_put_variable_param!(exo_id, EX_NODAL, num_vars)
end

function write_nodal_variable_names(exo_id::int, var_indices::Vector{IntKind}, var_names::Vector{String})
...
function write_nodal_variable_values(exo_id::int, time_step::IntKind, 

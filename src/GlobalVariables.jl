...
function read_global_variable_names(exo::ExodusDatabase)
  ...
  error_code = @ccall libexodus.ex_get_variable_names(
    get_file_id(exo)::Cint, EX_GLOBAL::ex_entity_type, num_vars::Cint, var_names::Ptr{Ptr{UInt8}}
  )::Cint
  exodus_error_check(error_code, "Exodus.read_global_variable_names -> libexodus.ex_get_names")
...
"""
"""
function write_global_variable_names(exo::ExodusDatabase, var_names::Vector{String})
  error_code = @ccall libexodus.ex_put_variable_names(
    get_file_id(exo)::Cint, EX_GLOBAL::ex_entity_type, length(var_names)::Cint,
    var_names::Ptr{Ptr{UInt8}}
  )::Cint
  exodus_error_check(error_code, "Exodus.write_global_variable_names -> libexodus.ex_put_variable_names")
end
